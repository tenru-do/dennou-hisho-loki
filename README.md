...
- Codex作業状態のHUD通知と読み上げ
- 顔表情を切り替える単色緑マスコット
- 頭を上げたときのHUD表示、回答中の画面保持
...
2. スマホ側でカレンダー、マイク、通知、Health Connectのうち使う権限だけを許可します。
3. スマホ側の「指示」パネルに表示される32文字の連携トークンを確認します。
4. グラス側のGemini設定でAPIキー、任意のカスタム指示、同じ連携トークンを保存します。
5. 同じネットワーク上で接続を確認します。
...
- Codex通知は別途ローカル監視側からスマホの `/codex` へ送る構成が必要です。
